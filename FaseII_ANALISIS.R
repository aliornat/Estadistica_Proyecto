...
#0. COMPROBAR DATOS
colnames(goodreadsdatos) #Todas las variables necesarias están presentes
sapply(goodreadsdatos[, c("average_rating", "ratings_count", "text_reviews_count", "num_pages", "year_published")], function(x) sum(is.na(x)))
#porque se han encontrado dos variables nulas en year_published
goodreadsdatos <- goodreadsdatos%>%
  filter(!is.na(year_published))
sum(is.na(goodreadsdatos$year_published)) #devuelve 0

#1. Regresión lineal múltiple
#escalar variables
datos_escalados <- goodreadsdatos %>%
  mutate(across(c(ratings_count, text_reviews_count, num_pages, year_published), scale))
str(datos_escalados)

#preparar modelo
modelo_regresion <- lm(average_rating ~ ratings_count + text_reviews_count + num_pages + year_published, 
                       data = datos_escalados)
summary(modelo_regresion)

# Calcular valores predichos y residuos
datos_diagnostico <- data.frame(
  predichos = modelo_regresion$fitted.values,
  residuos = modelo_regresion$residuals
)

ggplot(datos_diagnostico, aes(x = predichos, y = residuos)) +
  geom_point(color = "#FF1493", alpha = 0.7) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "#A6C9E2") +
  labs(title = "Gráfico de Residuales vs Predichos",
       x = "Valores Predichos",
       y = "Residuales") +
  theme_minimal() +
...
ggplot(datos_diagnostico, aes(sample = residuos)) +
  stat_qq(color = "#FF1493") +
  stat_qq_line(color = "#A6C9E2") +
...
#quitar text_reviews_count porque es muy poco significativa
modelo_simplificado <- lm(average_rating ~ ratings_count + num_pages + year_published, 
                          data = datos_escalados)
summary(modelo_simplificado)
anova(modelo_simplificado)

# Calcular los valores predichos y los residuos para el modelo simplificado
datos_diagnostico_simplificado <- data.frame(
  predichos = modelo_simplificado$fitted.values,
  residuos = modelo_simplificado$residuals
)

ggplot(datos_diagnostico_simplificado, aes(x = predichos, y = residuos)) +
  geom_point(color = "#FF1493", alpha = 0.7) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "#A6C9E2") +
  labs(
    title = "Gráfico de Residuales vs Predichos (Modelo Simplificado)",
    x = "Valores Predichos",
    y = "Residuales"
  ) +
  theme_minimal() +
...
# QQ-Plot de los residuos
ggplot(datos_diagnostico_simplificado, aes(sample = residuos)) +
  stat_qq(color = "#FF1493") +
  stat_qq_line(color = "#A6C9E2") +
  labs(
    title = "QQ-Plot de los Residuos (Modelo Simplificado)",
    x = "Cuantiles Teóricos",
    y = "Cuantiles de los Residuos"
  ) +
  theme_minimal() +
...
# Si 'datos_escalados' ya es una matriz
if (!is.matrix(datos_escalados)) {
  datos_escalados <- as.data.frame(datos_escalados)  # Convertir a data frame si no lo es
  datos_escalados <- datos_escalados %>% select_if(is.numeric)  # Filtrar columnas numéricas
  datos_escalados <- as.matrix(datos_escalados)  # Convertir de vuelta a matriz si necesario
}

# Verificar que todo es numérico
if (!all(apply(datos_escalados, 2, is.numeric))) {
  stop("Los datos contienen columnas no numéricas, revisa tu dataset.")
}
...
for (i in 1:15) {
  wcss[i] <- sum(kmeans(datos_escalados, centers = i, nstart = 10)$withinss)
}

# Método del codo
library(ggplot2)
ggplot() + 
  geom_point(aes(x = 1:15, y = wcss), color = "#FF1493") + 
  geom_line(aes(x = 1:15, y = wcss), color = "#A6C9E2") + 
  ggtitle("Método del Codo para Determinar el Número de Clusters") +
  xlab("Número de Clusters (k)") +
  ylab("WCSS (Suma de Cuadrados Intra-Cluster)")


#KMEANS K=4
set.seed(1234)
k_optimo <- 4  # Cambia este valor según lo que determine el método del codo
modelo_kmeans <- kmeans(datos_escalados, centers = k_optimo, nstart = 10)

# Añadir la asignación de clusters al dataset original
datos_escalados <- as.data.frame(datos_escalados)  # Convertir a data frame si es necesario
datos_escalados$cluster <- as.factor(modelo_kmeans$cluster)

# Mostrar el resultado
print(modelo_kmeans)
print(table(datos_escalados$cluster))  # Tamaño de cada cluster

ggplot(datos_escalados, aes(x = datos_escalados[, 1], y = datos_escalados[, 2], color = cluster)) +
  geom_point(size = 2) +
  scale_color_manual(values = c("#FF1493", "#A6C9E2", "#FF9F94", "#FF9BFF")) +
  ggtitle(paste("Clusters Visualizados con K-Means (k =", k_optimo, ")")) +
  xlab("Promedio de puntuación") +
  ylab("Número de páginas") +
  theme_minimal()

# Calcular la distancia entre los puntos
distancias <- dist(datos_escalados)

# Calcular el índice de silueta
library(cluster)
silhouette_kmeans <- silhouette(modelo_kmeans$cluster, distancias)

# Visualizar y resumir el índice
plot(silhouette_kmeans, col = 1:4, border = NA)
summary(silhouette_kmeans)


#cluster 3
aggregate(datos_escalados, by = list(cluster = modelo_kmeans$cluster), FUN = mean)
datos_con_clusters <- datos_escalados
datos_con_clusters$cluster <- as.factor(modelo_kmeans$cluster)

install.packages("gridExtra")
library(gridExtra)


gc1 <- ggplot(datos_con_clusters, aes(x = cluster, y = num_pages)) +
  geom_boxplot() +
  ggtitle("Comparación de num_pages entre Clusters")

gc2 <- ggplot(datos_con_clusters, aes(x = cluster, y = ratings_count)) +
  geom_boxplot() +
  ggtitle("Comparación de ratings_count entre Clusters")

gc3 <- ggplot(datos_con_clusters, aes(x = cluster, y = year_published)) +
  geom_boxplot() +
  ggtitle("Comparación de year_published entre Clusters")

gc4 <- ggplot(datos_con_clusters, aes(x = cluster, y = text_reviews_count)) +
  geom_boxplot() +
  ggtitle("Comparación de text_reviews_count entre Clusters")
...
variables <- datos_con_clusters[, c("num_pages", "ratings_count", "year_published", "text_reviews_count")]
variables <- data.frame(lapply(variables, as.numeric)) # Asegurarse de que son numéricas
head(variables)

norm_var <- scale(variables)
pca <- prcomp(norm_var, center = TRUE, scale. = TRUE)
summary(pca)
...
fviz_pca_ind(pca,
             geom.ind = "point", # Representa cada observación como un punto
             col.ind = as.factor(datos_con_clusters$cluster), # Colorear por clusters
             palette = c("#FF1493", "#A6C9E2", "#FF9F94", "#FF9BFF"), # Colores personalizados
             addEllipses = TRUE, # Añadir elipses alrededor de cada grupo
             legend.title = "Clusters",
             repel = TRUE # Evita solapamientos de etiquetas
)
# Gráfico de las variables (cargas de las componentes)
fviz_pca_var(pca, col.var = "steelblue", repel = TRUE)

fviz_pca_biplot(pca, 
                geom.ind = "point",
                col.ind = as.factor(datos_con_clusters$cluster), # Colorear por clusters
                palette = c("#FF1493", "#A6C9E2", "#FF9F94", "#FF9BFF"),
                addEllipses = TRUE,
                legend.title = "Clusters",
                col.var = "black", # Variables en negro
                repel = TRUE)
